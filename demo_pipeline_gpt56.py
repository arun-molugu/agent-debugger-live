...
def _text(content) -> str:
    """gpt-5.6 via Responses API returns structured blocks; extract plain text."""
...
def order_lookup_node(state: PipelineState) -> dict:
    ...
    summary = _text(llm.invoke(prompt).content)       # LIVE model decision
...
def fulfillment_node(state: PipelineState) -> dict:
    ...
    assessment = _text(llm.invoke(prompt).content)    # LIVE model decision
...
def email_node(state: PipelineState) -> dict:
    ...
    email = _text(llm.invoke(prompt).content)         # LIVE model decision
...
def supervisor_node(state: PipelineState) -> dict:
    ...
    note = _text(llm.invoke(prompt).content)          # LIVE model decision
...
if __name__ == "__main__":
    print("=" * 66)
    print("  REAL 4-AGENT PIPELINE (GPT-5.6-terra) - order ORD-4471 (not in database)")
    print("=" * 66)
